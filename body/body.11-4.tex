  ...
  \fig{figs/fig-2ff72db7.png}{Figure 3. The impulse response corresponding to 
  the clarinet impedance shown in Fig. 1, computed from the fitted modal 
  parameters.} 

  \fig{figs/fig-c4746eeb.png}{Figure 4. The impulse response corresponding to 
  the saxophone impedance shown in Fig. 2, computed from the fitted modal 
  parameters.} 
  ...
  \fig{figs/fig-7b9953b8.png}{Figure 5. The modal frequencies for the clarinet 
  (red curve) and the soprano saxophone (blue curve) deduced from modal 
  fitting, each normalised by the fundamental frequency. The ``error bars'' 
  indicate the half-power bandwidths of each modal peak. The lower green line 
  shows the behaviour expected if the frequencies were perfectly harmonic with 
  ratios 1,2,3,4... while the upper green line shows the corresponding 
  ...
  \fig{figs/fig-24e29975.png}{Figure 6. The Q-factors deduced from the modal 
  fitting process, for the clarinet (red) and the soprano saxophone (blue). The 
  dashed black curve shows corresponding Q-factors deduced from the idealised 
  clarinet impedance, shown in Fig. 1 of section 11.3.2 and used for the 
  clarinet simulations of section 11.3. The dotted black curve shows the 
  ...
  \fig{figs/fig-87894312.png}{Figure 7. The amplitude factors (squared modal 
  amplitudes) deduced from the modal fitting process, for the clarinet (red) 
  and the soprano saxophone (blue). The dashed black curve shows the values 
  used in the idealised clarinet model from section 11.3, and the dotted black 
  curve shows the corresponding idealised saxophone model.} 
  ...
  \fig{figs/fig-79a099d6.png}{Figure 18. Pressure-gap diagram for the saxophone 
  over the same ranges of mouth pressure and reed gap as the examples shown in 
  section 11.3. These simulations were all run assuming a ``cold start'', with 
  the mouth pressure suddenly applied at time $t=0$. The colour indicates which 
  register of the saxophone each note plays in. The assumed Q-factor of the 
  reed was 2.5. The three horizontal green lines mark the sets of waveforms to 
  be shown in Fig. 18, and the circles mark cases whose frequency spectra are 
  shown in Fig. 20. [Modal saxophone model using measured impedance, dynamic 
  reed model.]} 
  ...
  \fig{figs/fig-4ae784bb.png}{Figure 19. The final portion of the waveforms 
  along lines marked in green in Fig. 18, for the cases that produced notes. 
  The left-hand column corresponds to the upper line, the middle column to the 
  middle line, the right-hand column to the lower line: the format is the same 
  as Fig. 13. The second waveform from the top in the middle column is the case 
  shown in Fig. 17. [Modal saxophone model using measured impedance, dynamic 
  reed model.]} 
  ...
  \fig{figs/fig-27d82b74.png}{Figure 20. Frequency spectra of the steady 
  waveforms of runs marked by circles in Fig. 18. In the interests of clarity, 
  the spectra have been separated by steps of 40~dB. [Modal saxophone model 
  using measured impedance, dynamic reed model.]} 
  ...
  \fig{figs/fig-e670e301.png}{Figure 21. Pressure-gap diagram for the same 
  simulations as in Fig. 18, but now colour-shaded to show the pitch deviation 
  in cents from the nominal frequency, taking the different registers into 
  account. [Modal saxophone model using measured impedance, dynamic reed 
  model.]} 
  ...
  \fig{figs/fig-af7911eb.png}{Figure 22. Pressure-gap diagram as in Fig. 18, 
  but initialised in a way that encouraged oscillation at the fundamental 
  frequency. The resulting red wedge indicates the region in this plane in 
  which it is possible to maintain a steady note in the first register, once it 
  is started. The three horizontal green lines mark the sets of waveforms to be 
  shown in Fig. 25. [Modal saxophone model using measured impedance, dynamic 
  reed model.]} 
  ...
  \fig{figs/fig-1d8d72f3.png}{Figure 23. Pressure-gap diagram for the same 
  simulations as in Fig. 22, but now colour-shaded to show the deviation in 
  cents from the nominal frequency. [Modal saxophone model using measured 
  impedance, dynamic reed model.]} 

  \fig{figs/fig-b4e936b1.png}{Figure 24. Pressure-gap diagram for the same 
  simulations as in Fig. 22, but now colour-shaded to show the spectral 
  ...
  \fig{figs/fig-2f28a513.png}{Figure 25. The final portion of the waveforms 
  along lines marked in green in Fig. 22, for the cases that produced notes. 
  The left-hand column corresponds to the upper line, the middle column to the 
  middle line, the right-hand column to the lower line: the format is the same 
  as Fig. 13. [Modal saxophone model using measured impedance, dynamic reed 
  model.]} 
  ...
  \fig{figs/fig-86665834.png}{Figure 27. Pressure-gap diagram computed with the 
  multiphonic fingering giving the impedance in Fig. 26. The three circles mark 
  the cases used in Figs. 28 and 29, and in Sound 2. [Modal clarinet model 
  using measured impedance, dynamic reed model.]} 
  ...
  \fig{figs/fig-555b5b21.png}{Figure 28. The pressure waveforms from pixels 30, 
  28 and 26 in the bottom row if Fig. 27 (marked there by circles). [Modal 
  ...
  \fig{figs/fig-32a55546.png}{Figure 29. Frequency spectra of the three 
  waveforms shown in Fig. 28, calculated from the next half-second of the 
